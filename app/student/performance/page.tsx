  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        let user = null;
        try {
          // Try to get user but don't redirect if it fails
          user = await getCurrentUser()
        } catch (error) {
          console.error("Failed to get user, using mock data instead:", error);
        }
        
        // If no user is found, use a mock user
        if (!user) {
          user = {
            id: "demo-student",
            name: "Demo Student",
        ...
        // Fetch or mock student performance data
        try {
          const data = await getStudentPerformanceData(user.id)
          setPerformanceData(data)
        } catch (error) {
          console.error("Failed to get performance data, using mock data", error);
          // Will use mockPerformanceData as fallback
        }
      } catch (error) {
